...
int main(int argc, char *argv[]){    
    ...
    while(1){  
        ...
        //This creates the child process, and if fork() returns 0, it must be in the child.
        if(!fork()){ 
            // In the child you must close the main listener socket.
            // The child will use only connect_d socket to talk to the client.
            close(listener_d); 
            ...
            // That will prevent it from faliing into the main server loop.
            close(connect_d);  
            exit(0);      
        }
        close(connect_d);
